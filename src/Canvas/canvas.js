...
class Canvas extends Component {
	...
	canvasClick(event) {
		...
		switch (situation) {
			...
			case 'ending-edge':
				...
				this.setState({ flowArr });
				const edge = {
					from: this.state.flowArr[this.state.flowArr.length - 1].from.id,
					to: this.state.flowArr[this.state.flowArr.length - 1].toID,
					action: 'add-edge'
				};
	...
	deleteLastValueInFlowArr() {
		...
		if (flowArr.length > 2) {
			console.log(`Canvas deleteLastValueInFlowArr - larger than 2`);
		...
		if (this.props.action !== prevProps.action) {
          ...
          this.props.flowappFromCanvas('none');
				...
				case 'node':
					...
					break;
				case 'undo':
					console.log(`Canvas componentDidUpdate - action: UNDO`);
